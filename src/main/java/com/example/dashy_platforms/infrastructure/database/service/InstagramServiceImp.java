 ...
 */
    ...
    public InstagramTokenResponse.TokenData getInstagramToken(String code) {
        ...
        try {
            ResponseEntity<InstagramTokenResponse> response = restTemplate.postForEntity(url, request, InstagramTokenResponse.class);
            ...
            throw new RuntimeException("Instagram authentication failed: " + e.getMessage());
        }
    }
   ...
   user = userTrouvable.get();
    ...
    public InstagramUserEntity OuthentificationProcess(String code){

        InstagramTokenResponse.TokenData tokenData = getInstagramToken(code);

        InstagramLongLivedTokenResponse longLivedToken = exchangeForLongLivedToken(tokenData.getAccessToken());
